...
第三版依据《五部作品_阅读记录汇编》增加“阅读互照”，以 14 处具体情境连接价值根基与各章。场景、体系关联、未解张力与开放问题构成一组阅读；可展开 56 段汇编札记核对上下文。五部作品并列呈现，不当作体系的证明链条。第三版中的《哈姆雷特》仅采用后续回看；《红楼梦》止于前八十回。

第四版根据2026年9月6日《哈姆雷特完整阅读补记》新增独立深读区：六条主题路径、五幕二十场精读、七个人物、八则完读札记。三处补读提示回接体系原文段落13、62、97。完整补记可下载，原有两则后续回看及其他作品内容完整保留。

- 哲学底座：两个承认、两道墙与生命邀请，五个编辑层面的交互导读。
- 阅读互照：五部作品、原有14处情境与《哈姆雷特》完整深读；从哲学底座、体系地图和九章原文双向进入，逐处保留札记来源。
...
`app/literature-data.json` 保存新增编辑归纳与汇编原段。原有14处情境的 `quote` 均为阅读札记的连续节选，并非一律属于文学原著直接引文；`excerpts` 保留所引札记原段，编号来自汇编中非空正文段落的顺序，表格另行读取而不计入该编号。材料的阶段性对照不回写现有体系正文。

`app/hamlet-data.json` 保存与补记逐字一致的二十场札记和八篇思考；`app/hamlet-editorial.json` 保存六扇门互照、人物与三条引用补充。英文短引与Folger底本按场次连续核验。新补记不冒充先前缺失的旧札记，体系原稿保持原文。

《哈姆雷特》入口可用 `#literature-hamlet--hamlet-person`、`#literature-hamlet--hamlet-scene-3-2`、`#literature-hamlet--hamlet-person-ophelia`、`#literature-hamlet--hamlet-essay-6`。
...
验证了 TypeScript 编译、静态生产构建、原稿文字完整性、导读引用定位、页面资源引用与项目子路径构建。新增43个《哈姆雷特》入口、12个其他作品入口及3处补读提示通过非浏览器静态渲染检查。未执行浏览器交互或截图测试。WebMCP（网页模型上下文协议）提供一个可选的阅读导航接口；不支持该协议的浏览器照常使用网页。当前环境没有可用的协议验证上下文，未验证该可选接口的运行注册。
