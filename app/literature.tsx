...
export function Literature({workId,episodeId,open,read}:{workId:string;episodeId:string;open:OpenLiterature;read:(chapter:number,paragraph?:number)=>void}) {
 ...
 return <section className="literature-view">
  ...
  <Tabs value={work.id} onValueChange={v=>open(String(v))} className="literary-tabs">
   <TabsList className="literary-shelf" aria-label="选择作品">{literaryWorks.map(w=><TabsTrigger value={w.id} key={w.id}><span className="shelf-number">{w.number}</span><span><strong>{w.title}</strong><small>{w.id==='hamlet'?'后续回看':w.id==='red'?'前八十回':w.id==='poem'?'小序与全词':'阅读札记'}</small></span></TabsTrigger>)}</TabsList>
   {literaryWorks.map(w=><TabsContent value={w.id} key={w.id}>
    <div className="literary-book-intro"><div><span className="small-label">{w.author}</span><h2>{w.theme}</h2></div><div><p>{w.note}</p><span className="literary-scope">{w.scope}</span></div></div>
   </TabsContent>)}
  </Tabs>
  ...
  <p className="literary-editorial-note">这里呈现的是阅读札记与体系之间的编辑互照，不把人物等同于概念，也不把札记的判断称作作家的最终立场。体系正文沿用《自成其形_阅读修订稿》；汇编中的阶段性对照保留其当时版本。</p>
 </section>;
}
