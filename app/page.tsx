...
export default function Home() {
  ...
  const openLiterature = (work:string,id?:string) => {
    ...
    setLiteraryWork(validWork);setEpisode(entry.id);setView('literature');
    if(!sameWork)window.scrollTo({top:0});
    else if(window.innerWidth<=820)requestAnimationFrame(()=>document.querySelector('.literary-workspace')?.scrollIntoView({block:'start'}));
  };
